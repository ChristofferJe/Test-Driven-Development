...
public class DishEffectCardsStrategy implements CardsStrategy {
    ...
    public ArrayList<Card> generateCards(Player owner) {
        ...
        for(int i=0; i<2; i++){
            Card BrownRice = new StandardCard(GameConstants.BROWN_RICE_CARD, 1,1,1, owner);
            ...
            Card TomatoSalad = new StandardCard(GameConstants.TOMATO_SALAD_CARD, 2, 2, 2, owner);
            Card PokeBowl = new StandardCard(GameConstants.POKE_BOWL_CARD, 3, 2, 3, owner);
            ...
            Card ChickenCurry = new StandardCard(GameConstants.CHICKEN_CURRY_CARD,6,4,4, owner);
            Card BeefBurger = new StandardCard(GameConstants.BEEF_BURGER_CARD,6,8,6, owner);
